...
library ChessLogic {
    ...
    bytes constant c_Flags = "\x08\x09\x7b\x0b\x38\x4e\x4f\x3e\x3f\x3d\x4d"; // [  8,  9,123, 11, 56, 78, 79, 62, 63, 61, 77]
    ...
    function move(State storage self, uint256 fromIndex, uint256 toIndex, bool isWhite) public {
        ...
        if (abs(fromFigure) != uint(int(Pieces(Piece.WHITE_KNIGHT)))) {
            ...
            bool _checkForCheck = abs(fromFigure) == uint(int(Pieces(Piece.WHITE_KING)));
            checkWayFree(self, fromIndex, toIndex, currentPlayerColor, _checkForCheck);
    ...
    function sanityCheck(uint256 fromIndex, uint256 toIndex, int8 fromFigure, int8 toFigure, int8 currentPlayerColor) pure internal {
        ...
        if (toIndex & 0x88 != 0 || fromIndex & 0x88 != 0) {
            revert('Invalid Move: Out of field');
        }

        // check that from and to are distinct
        if (fromIndex == toIndex) {
            revert('Invalid Move: Cannot move to the same figure');
        }
        ...
        if (fromFigure * toFigure > 0) {
            revert('Invalid Move: Must be empty or with enemy figure');
        }
        ...
        if (currentPlayerColor * fromFigure <= 0) {
            revert('Invalid Move: Must be owner of the figure');
        }
    }
    ...
    function validateMove(State storage self, uint256 fromIndex, uint256 toIndex, int8 fromFigure, int8 toFigure, int8 movingPlayerColor) public returns (bool) {
        ...
        if (abs(fromFigure) == uint(int(Pieces(Piece.WHITE_KNIGHT)))) {
            for (uint i; i < 8; i++) {
                if (int(fromIndex) + int8(uint8(knightMoves[i])) - 64 == int(toIndex)) {
                    return true;
                }
            }
            return false;
    ...
    function checkForCheck(State storage self, uint256 kingIndex, int8 currentPlayerColor) internal returns (bool) {
        ...
        for (uint dir = 0; dir < 8; dir ++) {
            ...
            int8 firstFigureIndex = getFirstFigure(self, Directions(Direction(dir)), int8(int(kingIndex)));
            ...
            if (firstFigureIndex != -1) {
                ...
                if (firstFigure * currentPlayerColor < 0) {
                    ...
                    if (validateMove(self, uint256(int(firstFigureIndex)), uint256(kingIndex), firstFigure, kingFigure, currentPlayerColor)) {
                        // it can
        ...
        for (uint _move = 0; _move < 8; _move++){
            // currentMoveIndex: where knight could start with move that checks king
            int8 currentMoveIndex = int8(uint8(kingIndex)) + int8(uint8(knightMoves[_move])) - 64;
    ...
    function makeMove(State storage self, uint256 fromIndex, uint256 toIndex, int8 fromFigure, int8 toFigure) internal {
        ...
        if (abs(fromFigure) == uint(int(Pieces(Piece.WHITE_PAWN)))) {
            ...
            // in case of double Step: set EN_PASSANT-Flag
                ...
                if (fromFigure == Pieces(Piece.BLACK_PAWN)) {
                    setFlag(self, Flag.BLACK_EN_PASSANT, int8(int(toIndex)) + Directions(Direction.UP));
                } else {
                    setFlag(self, Flag.WHITE_EN_PASSANT, int8(int(toIndex)) + Directions(Direction.DOWN));
                }
            }
        }
    ...
    function checkLegality(State storage self, uint256 fromIndex, uint256 toIndex, int8 fromFigure, int8 movingPlayerColor) internal {
        // Piece that was moved was the king
        if (abs(fromFigure) == uint(int(Pieces(Piece.WHITE_KING)))) {
            if (checkForCheck(self, uint(toIndex), movingPlayerColor)) {
                revert('Invalid Move: Stilll in check');
            }
        ...
        if (checkForCheck(self, uint(int(kingIndex)), movingPlayerColor)) {
            revert('Invalid Move: Cannot move while in check');
        }


        // through move of fromFigure away from fromIndex,
        // king may now be in danger from that direction
        int8 kingDangerDirection = getDirection(uint256(int(kingIndex)), fromIndex);
        // get the first Figure in this direction. Threat of Knight does not change through move of fromFigure.
        ...
        if (firstFigureIndex != -1) {
            ...
            if (firstFigure * movingPlayerColor < 0) {
                ...
                if (validateMove(self, uint256(int(firstFigureIndex)), uint256(int(kingIndex)), firstFigure, kingFigure, movingPlayerColor)) {
                    // it can
                    revert('Invalid Move!');
                }
            }
